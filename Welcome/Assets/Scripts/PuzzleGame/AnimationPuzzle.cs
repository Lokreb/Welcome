...
public class AnimationPuzzle : MonoBehaviour
{
    ...
    {
        float distance = transform.localPosition.x * .25f;
        ...
        sequence.Append(transform.DOLocalJump(new Vector2(distance*3f,0f),transform.localPosition.y+hauteur,1,_vitesse2*.5f).SetEase(Ease.Linear));
        sequence.Append(transform.DOLocalJump(new Vector2(distance*2f,0f),hauteur*2f,1,_vitesse2*.20f).SetEase(Ease.Linear));
        sequence.Append(transform.DOLocalJump(new Vector2(distance*1f,0f),hauteur*.5f,1,_vitesse2*.20f).SetEase(Ease.Linear));
